...
class cConnection {
  ...
  function open($persistent = FALSE) {
    ...
    $this->_con = mysql_connect($this->_hostname.':'.$this->_port, $this->_username, $this->_password, true);
    return isset($this->_con) && $this->_con;
  }
  ...
  function importSql($database, $file) {
    `mysql -h{$this->_hostname} -u{$this->_username} -p{$this->_password} -P{$this->_port} {$database} < $file`;
  }
