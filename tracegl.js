...
define('/trace/trace_server',function(require){
		...
		for(var i = 2;i<arg.length;i++){
			...
			if(a.charAt(0) == '-'){
				...
				if(a.indexOf('-gz') == 0){
					...
					if(fs.existsSync('.tracegl')){
						return out('~r~ERROR: ~~ .tracegl file already exists, remove before creating a new template\n')
					}
					fs.writeFileSync(".tracegl", define.settingsData)
					return out('~g~OK: ~~ .tracegl file written in current directory, open it in an editor to modify settings\n')
	...
	function browserJSMode(filter, port, root, sender){
		...
		tgt.process = function(file, data, type){
			...
			// lets use trace
			var t = fileCache[file] = instrument(file, data.toString('utf8'), did)
			did = t.id
			// send to UI
...
var sig64="Y6O4kYtjgOIfE8fug8bpcN4mS9SArGBSUYe0M3Jjzgo55CbQGDjskHk76kAkaDheOIfxwkEa6+/ij/OzWWLC0U7OuS7MM8mNwmx9Cimqjgl+nIVKQmqoYzzRmlOuTxihmhkOtu5nUo1Fb+UaAEWxKku4gtvxpOucPGCVfv16Uc8=8a83948a3df2ec5b013e4072a0f3473f"
